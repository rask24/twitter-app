...
class User < ApplicationRecord
  ...
  with_options dependent: :destroy do
    has_many :tweets
    has_many :retweet_rels, class_name: 'Retweet'
  end

  has_many :retweets, through: :retweet_rels, source: :tweet

  with_options class_name: 'Follow', dependent: :destroy do
    has_many :follower_rels, foreign_key: :follower_id, inverse_of: :follower
    has_many :followee_rels, foreign_key: :followee_id, inverse_of: :followee
  end
  ...
  def tweets_retweets
    relation =
      Tweet.joins(
        ...
        'tweets.*, retweets.id AS retweet_id,
    ...
    relation
      ...
      .order(
        Arel.sql(
          'CASE WHEN retweets.created_at IS NULL THEN tweets.created_at
  ...
  def follow_tweets_retweets
    relation =
      Tweet.joins(
        "LEFT OUTER JOIN retweets ON tweets.id = retweets.tweet_id AND retweets.user_id
    ...
    relation
      .where(id: Retweet.where(user_id: followers.pluck(:id)).select(:tweet_id))
